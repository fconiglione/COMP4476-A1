...
public class QuestionTwo {
    ...
    public static String encrypt(String text, int[] key, int m) {
        StringBuilder fittedText = new StringBuilder(text);
        // Add extra space to ensure text fits within the permutation size (m)
        while (fittedText.length() % m != 0) {
            fittedText.append(" ");
        }
        ...
        for (int i = 0; i < fittedText.length(); i += m) {
            char[] block = new char[m];
            for (int j = 0; j < m; j++) {
                block[key[j]] = fittedText.charAt(i + j);
            }
    ...
    public static String decrypt(String text, int[] key, int m) {
        // Create the decryption key for decryption
        int[] decryptionKey = new int[m];
        for (int i = 0; i < m; i++) {
            decryptionKey[key[i]] = i;
        }
        ...
        for (int i = 0; i < text.length(); i += m) {
            char[] block = new char[m];
            for (int j = 0; j < m; j++) {
                block[decryptionKey[j]] = text.charAt(i + j);
            }
            result.append(new String(block));
        }
